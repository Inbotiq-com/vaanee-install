...
-- ============================================================
-- organizations  (must come first — referenced by other tables)
-- ============================================================
...
-- Safe column additions for admins (idempotent)
ALTER TABLE admins ADD COLUMN IF NOT EXISTS is_temporary_password BOOLEAN DEFAULT false;
ALTER TABLE admins ADD COLUMN IF NOT EXISTS admin_id VARCHAR;
...
-- Safe column additions for users (idempotent)
...
-- ============================================================
-- subscription_execution_usage
-- ============================================================
CREATE TABLE IF NOT EXISTS subscription_execution_usage (
    id UUID PRIMARY KEY DEFAULT gen_random_uuid(),
    organization_id UUID NOT NULL REFERENCES organizations(id),
    execution_id UUID,
...
-- NOTE: flow_id column added for webhook compatibility
--       UNIQUE constraint on agent_id required for ON CONFLICT upsert
-- ============================================================
...
-- Safe additions for caller_ai_agent_flow_published (idempotent)
ALTER TABLE caller_ai_agent_flow_published ADD COLUMN IF NOT EXISTS flow_id UUID;

-- Backfill flow_id from source_flow_id for existing rows
UPDATE caller_ai_agent_flow_published SET flow_id = source_flow_id WHERE flow_id IS NULL;

-- Unique constraint on agent_id (required for ON CONFLICT upsert in publish flow)
DO $$ BEGIN
    IF NOT EXISTS (
...
-- Drop the foreign key constraint that blocks publishing flow
-- (published_flow_id references caller_ai_agent_flow_published but that table
--  may not exist yet when agent profile is created, causing FK violation)
ALTER TABLE caller_ai_agent_profiles DROP CONSTRAINT IF EXISTS caller_ai_agent_profiles_published_flow_fkey;
...
-- Drop incorrect unique constraint on campaigns (allows multiple campaigns per org)
ALTER TABLE caller_ai_campaigns DROP CONSTRAINT IF EXISTS caller_ai_campaigns_org_unique;
