...
INBOTIQ_API="https://app.inbotiq.com/api"  # your main platform API
...
$VAANEE_DOMAIN {
    ...
    # Webhook routes → webhook service
    handle /webhook/* {
...
services:
  ...
  vaanee-backend:
    image: $REGISTRY/vaanee-backend:latest
  ...
  vaanee-webhook:
    image: $REGISTRY/vaanee-webhook:latest
  ...
  vaanee-frontend:
    image: $REGISTRY/vaanee-frontend:latest
...
pull_images() {
    ...
    # Registry login using the API key as password
    echo "$VAANEE_API_KEY" | docker login "$REGISTRY" \
        --username vaanee-client \
...
CLI
    ...
    chmod +x /usr/local/bin/vaanee 2>/dev/null || true
